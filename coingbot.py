...
async def on_ready():
    ...
    await client.change_presence(game=discord.Game(name="pycoingecko"))
    ...
    if message.content.startswith(PREFIX):
        ...
        if match:
            ...
            if symbol not in COINS:
                res = "```This coin is not listed on exchanges we support 😬️```"
            else:
        ...
        if msg == 'help':
            res = "```GeckoPriceBot Commands: \n .p <coin ticker>: shows current price\n .r <coin ticker>: shows coin rank\n .c <coin ticker> <pairing> <time frame(hours)>: shows chart Donate 💖️ To Bot 3MCwFDDDEkzhAf24bavWhY4vtJHKB9VeEQ Powered by https://CoinGecko.com API\n```"
            await client.send_message(message.channel, res) 
